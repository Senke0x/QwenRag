...
def text_search(query: str, metadata_list: List[ImageMetadata], top_k: int) -> List[SearchResult]:
    """基于文本描述的搜索 (简单实现)"""
    results = []
...
def image_search(image_path: str, metadata_list: List[ImageMetadata], processor: ImageProcessor, top_k: int) -> List[SearchResult]:
    """基于图片的搜索 (简单实现)"""
    try:
        # 处理查询图片
...
def format_results(results: List[SearchResult], output_format: str, show_similarity: bool = False):
    """格式化输出结果"""
    if not results:
        print("没有找到匹配的结果")
        return
    ...
    if output_format == "json":
        ...
        for i, result in enumerate(results, 1):
            ...
            if meta.description:
                print(f"     描述: {meta.description[:80]}{'...' if len(meta.description) > 80 else ''}")
