    ...
    try:
        ...
        if 'retry' in yaml_data:
            ...
            config.retry = RetryConfig(
                ...
                exponential_base=retry_data.get('exponential_base', config.retry.exponential_base),
                retryable_errors=retry_data.get('retryable_errors', config.retry.retryable_errors)
            )
        ...
        if 'image_processor' in yaml_data:
            ...
            config.image_processor = ImageProcessorConfig(
                supported_formats=img_data.get('supported_formats', config.image_processor.supported_formats),
                max_image_size=img_data.get('max_image_size', config.image_processor.max_image_size),
                max_resolution=tuple(img_data.get('max_resolution', config.image_processor.max_resolution)),
                resize_quality=img_data.get('resize_quality', config.image_processor.resize_quality)
            )
