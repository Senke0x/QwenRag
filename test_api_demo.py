...
def test_text_search():
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE_URL}/api/v1/search/text",
            json=search_data
        )
        ...
        if result.get('results'):
            ...
            for i, item in enumerate(result['results'][:3]):
                print(f"  {i+1}. {item['image_info']['filename']} (相似度: {item['similarity_score']:.3f})")
...
def test_image_upload_and_search():
    ...
    try:
        ...
        with open(test_image, 'rb') as f:
            ...
            response = requests.post(
                f"{API_BASE_URL}/api/v1/search/image",
                files=files,
                params=params
            )
...
def test_face_detection():
    ...
    try:
        ...
        with open(test_image, 'rb') as f:
            ...
            response = requests.post(
                f"{API_BASE_URL}/api/v1/faces/detect",
                files=files
            )
        ...
        if response.status_code == 200:
            ...
            if face_count > 0:
                ...
                for i, face in enumerate(result.get('faces', [])):
                    ...
                    print(f"  人脸{i+1}: 位置({bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]}) 置信度:{face['confidence']:.3f}")
...
def test_index_build():
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE_URL}/api/v1/index/build",
            json=build_data,
            timeout=300  # 5分钟超时
        )
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"构建完成: 处理了{result.get('processed_images', 0)}/{result.get('total_images', 0)}张图片")
