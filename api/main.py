...
from api.routers import faces, indexing, search
...
def register_routers(app: FastAPI):
    """注册路由"""
    app.include_router(indexing.router, prefix="/api/v1/index", tags=["索引管理"])

    app.include_router(search.router, prefix="/api/v1/search", tags=["搜索"])

    app.include_router(faces.router, prefix="/api/v1/faces", tags=["人脸识别"])
...
def setup_exception_handlers(app: FastAPI):
    ...
    async def value_error_handler(request: Request, exc: ValueError):
        ...
        return JSONResponse(
            status_code=400, content={"error": "参数错误", "detail": str(exc)}
        )
    ...
    async def file_not_found_handler(request: Request, exc: FileNotFoundError):
        ...
        return JSONResponse(
            status_code=404, content={"error": "文件未找到", "detail": str(exc)}
        )
    ...
    async def general_exception_handler(request: Request, exc: Exception):
        ...
        return JSONResponse(
            status_code=500, content={"error": "服务器内部错误", "detail": "请稍后重试"}
        )
