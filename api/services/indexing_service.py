...
class IndexingService:
    ...
    def __init__(
        self, indexing_pipeline: IndexingPipeline, face_processor: FaceProcessor
    ):
    ...
    def _assess_index_health(
        self, image_stats: Dict[str, Any], face_stats: Dict[str, Any]
    ) -> str:
        """评估索引健康状态"""
        try:
    ...
    async def build_index(
        ...
        try:
            ...
            supported_extensions = [
                f".{ext}" for ext in get_supported_image_extensions()
            ]
            image_files = []
            ...
            processing_result = self.indexing_pipeline.process_image_batch(
                image_paths=image_paths, parallel=max_workers > 1
            )
            ...
            if process_faces:
                try:
                    ...
                    face_processing_result = (
                        self.indexing_pipeline.process_pending_face_embeddings()
                    )
    ...
    async def add_images(
        self, image_paths: List[str], process_faces: bool = True
    ) -> Dict[str, Any]:
        """
        添加图片到现有索引
        ...
        try:
            ...
            if not valid_paths:
                return {
                    ...
                    "image_details": [
                        {"path": path, "status": "not_found"} for path in invalid_paths
                    ],
                }

            # 执行批量处理
            processing_result = self.indexing_pipeline.process_image_batch(
                image_paths=valid_paths, parallel=True
            )
    ...
    async def clear_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            clear_result = {"cleared_items": [], "errors": []}
            ...
            if task_id in self.background_tasks:
                ...
                if "start_time" in task_info:
                    if task_info["status"] == "running":
                        task_info["running_time"] = (
                            time.time() - task_info["start_time"]
                        )
    ...
    async def optimize_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_result = {"optimized_components": [], "improvements": {}}
