...
async def detect_faces(
    ...
    try:
        ...
        response = await face_service.detect_faces(
            image_data=file_data, filename=file.filename
        )
...
async def search_similar_faces(
    ...
    try:
        ...
        response = await face_service.search_similar_faces(
            image_data=file_data, filename=file.filename, request=search_request
        )
...
async def get_face_statistics(face_service: FaceService = Depends(get_face_service)):
    """
    获取人脸识别统计信息
    ...
    try:
        ...
        return JSONResponse(
            status_code=200,
            content={"success": True, "message": "人脸识别统计信息获取成功", "statistics": stats},
        )
