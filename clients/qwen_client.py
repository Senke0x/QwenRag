...
from utils.retry_utils import NonRetryableError, RetryableError, retry_with_backoff
...
class QwenClient:
    ...
    def __init__(
        ...
        try:
            if (
                hasattr(root_config, "config")
                and root_config.config
                and hasattr(root_config.config, "qwen_vl")
            ):
        ...
        try:
            if (
                hasattr(root_config, "config")
                and root_config.config
                and hasattr(root_config.config, "retry")
            ):
        ...
        # 初始化OpenAI客户端（兼容模式）
        self.client = OpenAI(
            api_key=self.qwen_config.api_key, base_url=self.qwen_config.base_url
        )
    ...
    def _log_request_response(
        self, request_data: Dict[str, Any], response_content: str, method: str
    ):
        """
        记录请求和响应日志
...
"""
    ...
    @retry_with_backoff()
    def chat_with_text(
        self, user_prompt: str, system_prompt: Optional[str] = None, **kwargs
    ) -> str:
        """
        纯文本聊天
    ...
    def get_text_embedding(
        self, text: str, model: str = "text-embedding-v4"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if resp.status_code == HTTPStatus.OK:
                ...
                self._log_request_response(
                    request_data, response_content, "get_text_embedding"
                )
    ...
    def get_image_embedding(
        self, image_base64: str, model: str = "multimodal-embedding-v1"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            resp = dashscope.MultiModalEmbedding.call(model=model, input=input_data)
            ...
            if resp.status_code == HTTPStatus.OK:
                ...
                response_content = f"embedding_dimension: {len(embedding_vector)}, model: {model}, status: {resp.status_code}"
                self._log_request_response(
                    request_data, response_content, "get_image_embedding"
                )
    ...
    def _crop_face_from_base64(
        self, image_base64: str, face_rect: Dict[str, int]
    ) -> str:
        """
        从 base64 图片中裁剪人脸区域
