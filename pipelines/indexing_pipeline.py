...
class IndexingPipeline:
    ...
    def scan_image_directory(self, directory_path: str, recursive: bool = True) -> List[str]:
        """
        扫描目录获取所有支持的图片文件
    ...
    def _process_single_image_complete(self, image_path: str) -> Tuple[bool, Dict[str, Any]]:
        """
        完整处理单张图片：分析 + 向量化 + 存储
        ...
        try:
            ...
            # 统计向量化结果
            successful_embeddings = sum(1 for _, success in embedding_results if success)
    ...
    def process_image_batch(self, image_paths: List[str], parallel: bool = True) -> Dict[str, Any]:
        """
        批量处理图片
    ...
    def build_index_from_directory(
        ...
        try:
            ...
            if resume_from_metadata:
                ...
                image_paths = [path for path in image_paths if path not in processed_paths]
                logger.info(f"跳过已处理的 {len(processed_paths)} 个文件，剩余 {len(image_paths)} 个待处理")
            ...
            for i in range(0, len(image_paths), self.batch_size):
                ...
                logger.info(f"批次 {batch_num} 完成，成功: {batch_results['success']}, 失败: {batch_results['failed']}")
    ...
    def save_metadata(self) -> bool:
        ...
        try:
            ...
            new_metadata = [
                item for item in self.metadata_storage
