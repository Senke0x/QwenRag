...
class FaceProcessor:
    ...
    def __init__(
        ...
        self.face_store = face_store or FaceFaissStore(
            dimension=self.config.embedding_dimension, index_type="IndexFlatIP"
        )
    ...
    async def _extract_single_face_embedding(self, face_base64: str) -> np.ndarray:
        ...
        try:
            ...
            embedding = await self.qwen_client.get_image_embedding(
                image_base64=face_base64, instruction=prompt
            )
    ...
    async def process_image_faces(
        ...
        try:
            ...
            face_metadata_list = await self.extract_face_embeddings(
                image_metadata.path, image_metadata.face_rects, image_metadata.unique_id
            )
            ...
            for face_metadata in face_metadata_list:
                try:
                    self.face_store.add_face_embedding(
                        face_metadata.embedding_vector, face_metadata
                    )
        ...
        try:
            ...
            for query_face in query_face_embeddings:
                similar_faces = await self.find_similar_faces(
                    query_face.embedding_vector, top_k, similarity_threshold
                )
            ...
            for result in all_results:
                ...
                if (
                    face_id not in unique_results
                    or result.similarity_score
                    > unique_results[face_id].similarity_score
                ):
    ...
    def compare_faces(
        ...
        try:
            ...
            logger.info(
                f"人脸比较完成: {face_id1} vs {face_id2}, " f"相似度={similarity_score:.3f}"
            )
    ...
    async def batch_process_images(
        ...
        try:
            ...
            logger.info(
                f"批量人脸处理完成: {result.successful_extractions}/{result.total_images}张图片, "
