...
def validate_image_file(
    file_path: str, config: ImageProcessorConfig
) -> Tuple[bool, str]:
    """
    验证图片文件是否有效
...
def crop_face_from_image(
    image_path: str, face_rect: Tuple[int, int, int, int], padding: float = 0.2
) -> str:
    """
    从图片中裁剪人脸区域
        ...
        with Image.open(image_path) as img:
            ...
            if face_img.mode != "RGB":
                if face_img.mode in ("RGBA", "LA", "P"):
                    ...
                    background.paste(
                        face_img,
                        mask=face_img.split()[-1]
                        if face_img.mode in ("RGBA", "LA")
                        else None,
                    )
                    face_img = background
                else:
...
def find_images_in_directory(directory: str, config: ImageProcessorConfig) -> List[str]:
    ...
    try:
        for file_path in directory_path.rglob("*"):
            if file_path.is_file() and is_supported_image_format(
