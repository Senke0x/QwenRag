...
def test_qwen_client_basic():
    ...
    try:
        ...
        print(
            f"✅ 文本embedding成功: 维度={len(text_result['embedding'])}, 模型={text_result['model']}"
        )
...
def test_text_processing(processor):
    ...
    try:
        ...
        for result in search_results[:2]:  # 只显示前2个
            print(
                f"  - ID: {result['vector_id']}, 相似度: {result['similarity_score']:.4f}"
            )
...
def test_batch_processing():
    ...
    try:
        processor = EmbeddingProcessor(
            embedding_dimension=1536, index_save_path="test_data/batch_test_index"
        )

        # 批量文本处理
        batch_texts = ["海边的日落景色非常美丽", "雪山峰顶的壮丽景象", "城市繁华的商业街道", "宁静的乡村田园风光"]
...
def test_edge_cases():
    ...
    try:
        processor = EmbeddingProcessor(
            embedding_dimension=1536, index_save_path="test_data/edge_test_index"
        )
...
def test_workflow_simulation():
    ...
    try:
        print("📋 Step 1: 初始化处理器...")
        processor = EmbeddingProcessor(
            embedding_dimension=1536, index_save_path="test_data/workflow_index"
        )
        print("✅ 处理器初始化完成")

        print("\n📝 Step 2: 模拟文本数据处理...")
        sample_texts = ["风景如画的山水田园", "现代化的城市建筑群", "清晨的阳光洒向大地"]
...
def main():
    ...
    tests = [
        ...
        (
            "EmbeddingProcessor基本功能",
            lambda: test_embedding_processor_basic() is not None,
        ),
