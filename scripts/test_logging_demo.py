...
def test_image_chat_logging():
    ...
    with patch('clients.qwen_client.OpenAI') as mock_openai:
        ...
        mock_response.choices[0].message.content = '{"is_snap": false, "is_landscape": true, "description": "这是一张美丽的游戏截图，显示了《最后生还者》中的场景", "has_person": true, "face_rects": [[100, 150, 80, 120]]}'
...
def test_text_chat_logging():
    ...
    with patch('clients.qwen_client.OpenAI') as mock_openai:
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = "你好！我是通义千问，一个由阿里云开发的AI助手。我可以帮助你回答问题、进行对话、协助完成各种任务。有什么我可以帮助你的吗？"
        mock_client.chat.completions.create.return_value = mock_response
