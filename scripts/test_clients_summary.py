...
def test_prompt_manager():
    ...
    try:
        ...
        prompt_with_params = pm.get_prompt(
            PromptType.TEXT_GENERATION, user_request="测试请求"
        )
        assert "测试请求" in prompt_with_params["user"]

        # 测试自定义提示词
        pm.add_prompt("test_custom", "测试系统提示", "测试用户提示: {param}")
    ...
    try:
        ...
        with patch("clients.qwen_client.OpenAI") as mock_openai:
            ...
            mock_response.choices[
                0
            ].message.content = '{"is_snap": false, "is_landscape": true, "description": "测试图片", "has_person": false, "face_rects": []}'
            mock_client.chat.completions.create.return_value = mock_response
...
def test_error_handling():
    ...
    try:
        ...
        with patch("clients.qwen_client.OpenAI") as mock_openai:
            ...
            mock_client.chat.completions.create.side_effect = MockAuthError(
                "Unauthorized"
            )
