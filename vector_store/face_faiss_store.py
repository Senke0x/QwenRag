...
class FaceFaissStore(FaissStore):
    ...
    def __init__(
        ...
        logger.info(
            f"人脸向量存储初始化完成: dimension={dimension}, "
    ...
    def add_face_embedding(
        self, embedding: np.ndarray, face_metadata: FaceMetadata
    ) -> int:
        """
        添加人脸embedding到索引
        ...
        try:
            ...
            if len(embedding) != self.dimension:
                raise ValueError(
                    f"Embedding维度不匹配: 期望{self.dimension}, 实际{len(embedding)}"
                )
            ...
            logger.debug(
                f"添加人脸embedding成功: face_id={face_metadata.face_id}, "
                f"index_id={face_idx}"
            )
    ...
    def search_similar_faces(
        ...
        try:
            ...
            logger.info(
                f"人脸相似度搜索完成: 查询耗时{search_time:.3f}s, " f"返回{len(face_results)}个结果"
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "metadata_file_size": self.metadata_path.stat().st_size
                if self.metadata_path.exists()
                else 0,
