...
class TestImageProcessorRealAPI:
    ...
    def test_analyze_image_single_screenshot_real_api(self, real_processor, single_test_image):
        ...
        try:
            ...
            # 验证必需字段存在
            required_fields = ['is_snap', 'is_landscape', 'description', 'has_person', 'face_rects']
            ...
            if result['has_person'] and result['face_rects']:
                for face_rect in result['face_rects']:
                    ...
                    assert all(isinstance(coord, (int, float)) for coord in face_rect), "人脸框坐标应该是数字"
    ...
    def test_process_image_single_screenshot_real_api(self, real_processor, single_test_image):
        ...
        try:
            ...
            assert result.processing_status == ProcessingStatus.SUCCESS, f"处理状态应该是SUCCESS，实际是: {result.processing_status}"
            ...
            assert isinstance(result.last_processed, datetime), "last_processed应该是datetime类型"
    ...
    def test_process_images_batch_real_api(self, real_processor, dataset_images):
        ...
        try:
            ...
            assert len(results) == len(test_images), f"结果数量应该匹配输入，期望{len(test_images)}，实际{len(results)}"
            
            # 验证每个结果
            for i, (image_path, metadata) in enumerate(zip(test_images, results)):
                assert isinstance(metadata, ImageMetadata), f"第{i+1}个结果应该是ImageMetadata对象"
                assert metadata.path == image_path, f"第{i+1}个结果路径不匹配"
                
                # 验证处理状态（可能成功或失败）
                assert metadata.processing_status in [ProcessingStatus.SUCCESS, ProcessingStatus.FAILED], \
            ...
            for i, result in enumerate(results):
                status_symbol = "✓" if result.processing_status == ProcessingStatus.SUCCESS else "✗"
                image_name = os.path.basename(result.path)
                print(f"  {status_symbol} {i+1}. {image_name}: {result.processing_status}")
    ...
    def test_extract_face_embeddings_real_api(self, real_processor, dataset_images):
        ...
        try:
            ...
            assert metadata.processing_status == ProcessingStatus.SUCCESS, \
            ...
            if metadata.has_person and metadata.face_rects:
                ...
                assert len(face_embeddings) <= len(metadata.face_rects), \
                    "提取的人脸数量不应该超过检测到的人脸数量"
                
                # 验证每个embedding的格式
                for i, embedding in enumerate(face_embeddings):
                    assert isinstance(embedding, str), f"第{i+1}个人脸embedding应该是字符串(base64)"
                    assert len(embedding) > 0, f"第{i+1}个人脸embedding不应该为空"
                
                print(f"✓ 人脸提取成功: 检测到{len(metadata.face_rects)}个人脸，提取到{len(face_embeddings)}个embedding")
    ...
    def test_different_image_analysis_real_api(self, real_processor, dataset_images):
        ...
        try:
            ...
            for path, result in results:
                ...
                assert 'description' in result, f"{os.path.basename(path)}的结果缺少description"
                assert len(result['description']) > 0, f"{os.path.basename(path)}的description为空"
            
            print(f"✓ 多图片分析完成:")
            print(f"  图片1: {os.path.basename(image1_path)}")
            print(f"    描述: {result1['description'][:100]}...")
            print(f"    特征: 截图={result1['is_snap']}, 风景={result1['is_landscape']}, 人物={result1['has_person']}")
            
            print(f"  图片2: {os.path.basename(image2_path)}")
            print(f"    描述: {result2['description'][:100]}...")
            print(f"    特征: 截图={result2['is_snap']}, 风景={result2['is_landscape']}, 人物={result2['has_person']}")
    ...
    def test_image_metadata_consistency_real_api(self, real_processor, single_test_image):
        ...
        try:
            ...
            similarity_ratio = len(common_words) / max(len(desc1_words), len(desc2_words))
    ...
    def test_api_response_parsing_robustness_real_api(self, real_processor, single_test_image):
        ...
        try:
            ...
            # 验证默认值处理
            expected_keys = ['is_snap', 'is_landscape', 'description', 'has_person', 'face_rects']
