...
class TestPipelineIntegrationRealAPI:
    ...
    def test_indexing_pipeline_scan_directory_real_api(self, api_key_check, test_images_setup):
        """测试目录扫描功能 - 真实API"""
        pipeline = IndexingPipeline()
        ...
        for path in image_paths:
            ...
            assert Path(path).suffix.lower() in ['.jpg', '.jpeg', '.png'], f"不支持的图片格式: {path}"
    
    def test_indexing_pipeline_process_single_image_real_api(self, api_key_check, test_images_setup, temp_index_paths):
        """测试单张图片处理 - 真实API"""
        pipeline = IndexingPipeline(
    ...
    def test_indexing_pipeline_batch_processing_real_api(self, api_key_check, test_images_setup, temp_index_paths):
        """测试批量处理 - 真实API"""
        pipeline = IndexingPipeline(
            metadata_save_path=str(temp_index_paths["metadata_path"]),
            batch_size=2,
        ...
        print(f"批量处理完成: {results['success']}/{results['total']} 成功, 用时: {processing_time:.2f}秒")
    
    def test_retrieval_pipeline_initialization_real_api(self, api_key_check, temp_index_paths):
    ...
    @pytest.mark.slow
    def test_end_to_end_pipeline_flow_real_api(self, api_key_check, test_images_setup, temp_index_paths):
        """测试完整的端到端流程 - 真实API"""
        print("\n开始端到端流程测试...")
    ...
    def test_retrieval_pipeline_metadata_enhancement_real_api(self, api_key_check, temp_index_paths):
        """测试检索结果的元数据增强 - 真实API"""
        # 创建测试元数据
        test_metadata = [
            {
        ...
        # 测试unique_id提取
        assert pipeline._extract_unique_id_from_vector_id("test_img_001_full") == "test_img_001"
        assert pipeline._extract_unique_id_from_vector_id("test_img_001_desc") == "test_img_001"
        assert pipeline._extract_unique_id_from_vector_id("test_img_001_face_0") == "test_img_001"
