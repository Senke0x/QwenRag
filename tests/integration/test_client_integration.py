...
class TestClientIntegration:
    ...
    def test_image_processor_with_custom_clients(self):
        ...
        processor = ImageProcessor(
            qwen_client=qwen_client, prompt_manager=prompt_manager
        )
    ...
    def test_multiple_prompt_types_integration(self):
        ...
        for test_case in test_cases:
            ...
            for keyword in test_case["keywords"]:
                assert (
                    keyword in prompt["user"]
                ), f"Keyword '{keyword}' not found in {test_case['prompt_type'].value} prompt"
    ...
    def test_error_handling_integration(self):
        ...
        prompt_manager = PromptManager()
        processor = ImageProcessor(
            qwen_client=invalid_client, prompt_manager=prompt_manager
        )
    ...
    def test_custom_prompt_workflow(self):
        ...
        formatted_user = custom_prompts["user"].format(content_type="图片", aspect="色彩构成")
    ...
    def test_client_reuse_across_components(self):
        ...
        processor1 = ImageProcessor(
            qwen_client=shared_qwen_client, prompt_manager=shared_prompt_manager
        )
        ...
        def worker(worker_id):
            try:
                ...
                result = shared_client.chat_with_text(
                    user_prompt=f"线程 {worker_id} 的简短测试", max_tokens=50
                )
