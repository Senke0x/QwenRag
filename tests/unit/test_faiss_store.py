...
class TestFaissStore:
    ...
    def test_index_creation_with_data(self, sample_vectors, sample_ids, faiss_index_path):
        """测试带数据的索引创建"""
    ...
    def test_vector_insertion_duplicate_id(self, sample_vectors, sample_ids):
        ...
        # 尝试插入相同ID（默认更新策略）
        result2 = faiss_store.add_vectors(sample_vectors[1:2], [sample_ids[0]], update_existing=True)
        assert result2 == True
    ...
    def test_index_persistence_save_load(self, sample_vectors, sample_ids, faiss_index_path):
        """测试索引保存和加载"""
        # 创建并保存索引
