...
from config import ImageProcessorConfig, QwenVLConfig
...
def setup_logging(log_level: str = "INFO", log_file: str = None):
    """设置日志记录"""
    return setup_logger(
        name="qwen_rag_indexing", log_file=log_file, log_level=log_level
    )
...
def parse_arguments():
    ...
    parser.add_argument("--input_dir", "-i", required=True, help="输入图片目录路径")

    parser.add_argument("--output_dir", "-o", required=True, help="输出索引目录路径")

    parser.add_argument(
        "--api_key", "-k", help="Qwen API密钥 (也可通过DASHSCOPE_API_KEY环境变量设置)"
    )

    parser.add_argument(
        "--batch_size", "-b", type=int, default=10, help="批处理大小 (默认: 10)"
    )
    ...
    parser.add_argument("--log_file", help="日志文件路径 (默认: 只输出到控制台)")

    parser.add_argument("--resume", "-r", action="store_true", help="恢复之前中断的索引任务")

    parser.add_argument("--skip_existing", action="store_true", help="跳过已经处理过的图片")
    ...
    try:
        ...
        for i in range(0, len(image_files), args.batch_size):
            ...
            success_count = sum(
                ...
                if r.processing_status == ProcessingStatus.SUCCESS
            )
        ...
        successful_metadata = [
            m for m in all_metadata if m.processing_status == ProcessingStatus.SUCCESS
        ]
